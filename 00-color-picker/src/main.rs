...
use egui::{
    vec2, Button, CentralPanel, ColorImage, Context, Slider, TextureHandle, TextureOptions,
};
...
impl eframe::App for ColorPickerApp {
    fn update(&mut self, ctx: &Context, _frame: &mut eframe::Frame) {
        CentralPanel::default().show(ctx, |ui| {
            ...
            ui.image(texture, vec2(200.0, 100.0));
            ui.add_space(10.0);
            ui.add(Slider::new(&mut self.r, 0..=255).text("red"));
            ui.add(Slider::new(&mut self.g, 0..=255).text("green"));
            ui.add(Slider::new(&mut self.b, 0..=255).text("blue"));
            ui.add_space(10.0);
            ui.horizontal(|ui| {
                ...
                if ui
                    .add_enabled(self.valid_hex(), Button::new("convert hex to rgb"))
                    .clicked()
                {
                    self.hex_to_rgb();
                }
